...
const variants: Record<Variant, string> = {
  ...
  high: 'bg-amber-500/14 text-amber-400 border border-amber-500/38',
  medium: 'bg-blue-500/12 text-blue-400 border border-blue-500/35',
  ...
  standby: 'bg-amber-500/12 text-amber-400 border border-amber-500/35',
  processing: 'bg-blue-500/12 text-blue-400 border border-blue-500/35',
  offline: 'bg-white/8 text-white/45 border border-white/15',
  gold: 'bg-blue-500/12 text-blue-400 border border-blue-500/35',
  blue: 'bg-blue-500/12 text-blue-400 border border-blue-500/35',
  grey: 'bg-white/8 text-white/55 border border-white/15',
  review: 'bg-amber-500/12 text-amber-400 border border-amber-500/35',
  investigating: 'bg-amber-500/12 text-amber-400 border border-amber-500/35',
  ...
  pending: 'bg-blue-500/10 text-blue-300 border border-blue-400/30',
  inactive: 'bg-white/6 text-white/38 border border-white/12',
  draft: 'bg-white/8 text-white/55 border border-white/15',
  ...
  paused: 'bg-amber-500/12 text-amber-400 border border-amber-500/35',
};
...
export function Badge({ variant, label, dot = false, className }: BadgeProps) {
  ...
  return (
    <span className={cn('inline-flex items-center gap-1.5 px-2 py-0.5 text-[9.5px] font-bold tracking-[1px] uppercase rounded-[2px]', cls, className)}>
      {dot && <span className={cn('w-1.5 h-1.5 rounded-full bg-current flex-shrink-0', (variant === 'active' || variant === 'processing' || variant === 'running') && 'animate-pulse')} />}
      {label}
    </span>
  );
}
